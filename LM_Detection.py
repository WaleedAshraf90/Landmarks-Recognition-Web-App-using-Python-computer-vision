...
def image_processing(image):
    ...
    classifier = tf.keras.Sequential(
        [hub.KerasLayer(model_url, input_shape=img_shape + (3,), output_key="predictions:logits")]) # بجيبة عن طريق tensorflowhub 
    img = PIL.Image.open(image)
    img = img.resize(img_shape)
    img1 = img  # خدت نسخة علشان اعمل visulaztion
    img = np.array(img) / 255.0
    img = img[np.newaxis]
    result = classifier.predict(img) #علشان الموديل يقدر يفهم الصورة
    return labels[np.argmax(result)],img1  # argmax بيدينى احتمالية
    # بروح ابدلة من label علشان ارجع class name
...
# لازم احط الصورة بتاعى على الجهاز علشان الموديل يقدر يشوفها
def run():
    ...
    if img_file is not None:
        save_image_path = './Uploaded_Images/' + img_file.name  # هنا بحدد path علشان الصورة يقدر يرفعها علشان لما الصورة بتترفع على الموقع بتتخزن هنا علشان اقدر الموديل يعملها classification
